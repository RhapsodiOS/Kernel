...
static const char *
PIIXGetName(unsigned long pciID)
{
	int i;
	for (i = 0; piixDeviceTable[i].name; i++) {
...
static BOOL
PIIXVerifyID(unsigned long pciID)
{
	return (PIIXGetName(pciID) == 0) ? NO : YES;
}
...
- (BOOL) probePCIController:(IOPCIDeviceDescription *)devDesc
{
	...
	/*
	 * Verify the PCI ID to make sure we have an Intel PIIX class controller.
	 */
	if (PIIXVerifyID(_controllerID) == NO) {
	...
	IOLog("%s: %s PCI IDE Controller at Dev:%d Func:%d Bus:%d\n",
		[self name], PIIXGetName(_controllerID), devNum, funcNum, busNum);
